...
class KWR102(GPIBdev.GPIBdev):
    'Korad power supply class DC power supply class'

    def __init__(self, dev):
        super().__init__(dev , baud_rate = 115200, read_termination='\n', write_termination='\n')
        #self.inst.timeout = 10000
        # limits of the device
        self.vmin = +0
        self.vmax = +30
    ...
    def set_current(self, cur):
        self.gpib_write('ISET:%s' % (cur))

    def get_current(self):
        curM = self.gpib_query('ISET?')
        return curM

    def set_voltage(self, vol):
        self.gpib_write('VSET:%s' % (vol))

    def get_voltage(self):
        voltM = self.gpib_query('VSET?')
        return voltM
